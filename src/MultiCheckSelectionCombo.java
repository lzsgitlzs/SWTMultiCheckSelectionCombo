...
public class MultiCheckSelectionCombo extends Composite {
	...
	private void init() {
		...
		setLayout(layout);
		display = new Text(this, SWT.BORDER );
		display.setSize(200,25);
...
//	    });
	...
	private void showFloatShell(final Text display) {
		...
		final Shell shell = new Shell(MultiCheckSelectionCombo.this.getShell(), SWT.NO_TRIM);
...
//	   	});
		...
		shell.addListener(SWT.Deactivate, new Listener() {
			...
			public void handleEvent(Event event) {
				if (shell != null && !shell.isDisposed()) {
					...
					for (VerifyListener l : verifyListeners) {
						VerifyEvent v = new VerifyEvent(event);
						v.doit =false;
						l.verifyText(v);
					}
	...
	public int[] getSelectionIndices() {
		...
		Object[] s = selections.toArray();
		int[] ss = new int[selections.size()];
